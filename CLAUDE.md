...
tab in the iTerm2 window is doing — a name you can set, working directory and foreground
process for a plain tab and the file open in a Helix or mdview tab, working / waiting for
input / ready for a prompt for a Claude Code tab, or stale where the last thing reported has
stopped being credible, each with how long it has said so — and jumping focus to the row you
press Enter on.** Every phase says whether it produces one; a phase that does not is argued
for explicitly.
